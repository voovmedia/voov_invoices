...
@section('title')
    {{ __('auth.forgot_password.favicon_title') }}
@endsection
@section('content')
    <div class="d-flex flex-column flex-column-fluid align-items-center mt-12 p-4">
            ...
            @if (Session::has('status'))
                <div class="alert alert-success custom-message">
                    <div class="d-flex text-white align-items-center">
                        ...
                        <div>
                            <span class="text-white">{{ __('auth.forgot_password.email_success_msg') }}</span>
                        </div>
                    </div>
                </div>
            @endif
        ...
        <div class="bg-white rounded-15 shadow-md width-540 px-5 px-sm-7 py-10 mx-auto">
            <h1 class="text-center mb-7">{{ __('auth.forgot_password.heading_title') }}</h1>
            <div class="fw-bold fs-4 mb-5 text-center">{{ __('auth.forgot_password.email_reset_msg') }}</div>
                ...
                <div class="row">
                    <div class="mb-4">
                        ...
                        <input id="email" class="form-control" type="email" value="{{ old('email') }}" required
                            autofocus name="email" autocomplete="off" placeholder="{{ __('messages.common.email') }}" />
                    </div>
                </div>
                <div class="row">
                    <!-- Submit Field -->
                    <div class="form-group col-sm-12 d-flex text-start justify-content-center">
                        <button type="submit" class="btn btn-primary">
                            <span class="indicator-label"> {{ __('auth.forgot_password.email_pwd_reset_btn') }}</span>
                        </button>
                        <a href="{{ route('login') }}"
