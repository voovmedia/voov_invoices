                ...
                @foreach($quote->quoteItems as $quoteItem)
                    <tr class="tax-tr">
                        ...
                        <td class="table__item-desc w-25">
                            {{ Form::select('product_id[]', $products, isset($quoteItem->product_id)?$quoteItem->product_id:$quoteItem->product_name??[], ['class' => 'form-select productId client-product-quote io-select2', 'required', 'placeholder'=> __("messages.flash.select_product_or_enter_free_text"), 'data-control' => 'select2']) }}
