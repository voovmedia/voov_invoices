...
function loadCreateEditQuote () {
    ...
    if ($('#quoteNoteData').val() == true || $('#quoteTermData').val() ==
        true) {
        $('#quoteAddNote').hide();
...
function initializeSelect2CreateEditQuote () {
    ...
    $('#client_id').focus();
    let currentDate = moment(new Date()).add(1, 'days').format(convertToMomentFormat(currentDateFormat))
    ...
    let editQuoteDueDateFlatPicker = $('#clientEditQuoteDueDate').flatpickr({
        dateFormat: currentDateFormat,
    ...
    $('#client_quote_date').flatpickr({
        ...
        onChange: function () {
            let minDate
            if (currentDateFormat == 'd.m.Y' || currentDateFormat == 'd/m/Y' ||
    ...
    $('#clientEditQuoteDate').flatpickr({
        ...
        onReady: function () {
            ...
            if (currentDateFormat == 'd.m.Y' || currentDateFormat == 'd/m/Y' ||
                currentDateFormat == 'd-m-Y') {
                minDate2 = moment($('#clientEditQuoteDate').val(), convertToMomentFormat(currentDateFormat)).add(1, 'days').
                    format(convertToMomentFormat(currentDateFormat))
            } else {
                minDate2 = moment($('#clientEditQuoteDate').val()).add(1, 'days').
                    format(convertToMomentFormat(currentDateFormat))
            }
            if (typeof editQuoteDueDateFlatPicker != 'undefined') {
...
listenClick('#addClientQuoteItem', function () {
    ...
    let quoteItemHtml = prepareTemplateRender(
        '#quotesItemTemplate', data);
...
const calculateAndSetQuoteAmount = () => {
    ...
    $('.quote-item-container>tr').each(function () {
        ...
        quoteItemTotal = isEmpty($.trim(quoteItemTotal)) ? 0 : parseFloat(
            quoteItemTotal);
...
listenClick('#editSaveClientQuote', function (event) {
    ...
    let formData = $('#clientQuoteEditForm').serialize()+ '&quoteStatus=' +
        quoteStatus;
    screenLock();
    $.ajax({
